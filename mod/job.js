...
class Job {
  ...
  async deleteMesage() {
    await this.bot
      .execute("messages.getByConversationMessageId", {
  ...
  async canStartJob() {
    if (this.ctx.user.level < 0) {
      await this.cb.reply(
        "Ваш уровень слишком низкий. Извините, на данный момент вы не можете работать"
      );
      return false;
      ...
      [
        this.jobs.fishing.id,
        this.jobs.fishing.places.baikal.id,
        this.jobs.fishing.places.hafen.id,
      ].includes(this.ctx.cmd) &&
      this.ctx.user.level < this.jobs.fishing.level
    ) {
      return await this.cb.reply(
        `Простите, но рыбалка доступна с ${this.jobs.fishing.level} уровня.`
      );
    } else if (
      [
        this.jobs.fishing.places.baikal.id,
      ...
      this.ctx.user.energy < this.jobs.fishing.energy
    ) {
      return await this.cb.reply(
        `Вы устали, у вас ${this.ctx.user.energy} энергии ⏳ отдохните и возвращайтесь.`
      );
    } else if (
      [
        this.jobs.herbs.id,
      ...
      this.ctx.user.timers.hasWorked
    ) {
      return await this.cb.reply(
        `Вы уже работаете ${lang[this.ctx.user.timers.mainWork]}ом`
      );
    }
    return true;
  }
  ...
  async collect(
    ...
    this.ctx.user.skilsExp[expColl] =
      this.ctx.user.skilsExp[expColl] + 1 * mult;
    this.ctx.user.exp = this.ctx.user.exp + 1 * mult;
    await this.ctx.user.save();
    await this.ctx.user.inc("items", bait, "bait");
    await this.cb.reply(
      `Вы добыли ${earn} ${resIco} ${
        rRes > 0 ? `и ${rRes} ${rareResIco}️` : ""
      } ${bait > 0 ? `и ${bait} 🐛` : ""} у вас еще ${this.ctx.user.energy} ⚡`
    );
  }

  async fishing() {
    await this.cb.reply(
      "Вы отправляетесь на рыбалку, выберите место, куда идти"
    );
        ...
        [
          Markup.button({
            action: {
              ...
              payload: JSON.stringify({
                cmd: this.jobs.fishing.places.baikal.id,
              }),
            },
            color: "default",
          }),
          Markup.button({
            action: {
              ...
              payload: JSON.stringify({
                cmd: this.jobs.fishing.places.hafen.id,
              }),
            },
            color: "default",
          }),
  ...
  async fishingBaikal() {
    ...
    if (
      !this.ctx.user.invent.find(
        (x) => x.item.toString() === item._id.toString()
      )
    )
      return this.cb.reply("У вас нет удочки 🎣");
    if (this.ctx.user.items.bait === 0) {
      return this.cb.reply("У вас нет наживки 🐛");
    }
    ...
    await this.cb.reply(
      `Вы закинули удочку у вас еще ${this.ctx.user.energy} энергии.`
    );
    let massFish = [];
    let buttonMass = [];
    for (let i = 0; i < 10; i++) {
      ...
      } else if (randFish > 300 && randFish < 350) {
        massFish[i] = this.jobs.fishing.places.fishZ;
      } else if (randFish > 350 && randFish < 352) {
        massFish[i] = this.jobs.fishing.places.fishChest;
      } else {
        massFish[i] = this.jobs.fishing.places.fishX;
      }
      buttonMass[i] = Markup.button({
        action: {
          type: "callback",
          label: massFish[i].label,
          payload: JSON.stringify({ cmd: massFish[i].id, time: Date.now() }),
        },
        color: "default",
      });
    }
    await this.ctx.reply(
      `На рыбалку дается 1 вытягивание думай на что нажать.\nРыбалка на байкале:`,
      null,
      Markup.keyboard([
        [
          buttonMass[0],
  ...
  checkFishing() {
    if (this.ctx.user.items.bait < 1) {
      this.cb.reply("Недостаточно наживки 🐛");
      return false;
    } else if (
      this.ctx.message.createdTimestamp - this.ctx.message.payload.time >
        3600000 ||
      !this.ctx.message.payload.time
    ) {
      this.deleteMesage().then(
        this.cb.reply("Рыбка уже сгнила прошло больше часа")
      );
  ...
  collectBaikalX() {
    ...
    try {
      this.deleteMesage().then(async () => {
        ...
        await this.cb.reply(
          `Эххх ну так себе вы поймали ${earn} 🐟 у вас еще ${this.ctx.user.items.bait} наживки.`
        );
      });
    } catch (e) {
      console.log(e);
  ...
  collectBaikalY() {
    ...
    try {
      this.deleteMesage().then(async () => {
        ...
        await this.ctx.user.inc("inv", earn, "fish");
        await this.cb.reply(
          `Неплохо неплохо вы поймали ${earn} 🐟 у вас еще ${this.ctx.user.items.bait} наживки.`
        );
      });
    } catch (e) {
      console.log(e);
  ...
  collectBaikalZ() {
    ...
    try {
      this.deleteMesage().then(async () => {
        ...
        await this.cb.reply(
          `Уххх удачный улов вы поймали ${earn} 🐟 ${
            rare === 3 ? `и ${rFish} 🐡` : ""
          }у вас еще ${this.ctx.user.items.bait} наживки.`
        );
      });
    } catch (e) {
      console.log(e);
  ...
  collectChest() {
    ...
    try {
      this.deleteMesage().then(async () => {
        ...
        await this.cb.reply(
          `Вы поймали редкий сундук ${earn} ${lang.vinmt} ${
            rare < 2000 ? `и 1 🐡` : "и 1 ☘️"
          }у вас еще ${this.ctx.user.items.bait} наживки.`
        );
      });
